...
describe("Login page", () => {
  ...
  it(
    "Missing service identifier should show the correct error page (Finnish)",
    async () => {
      await browser.get(
        "http://localhost:3010/lang/fi/"
      );

      const title: string = await browser.getTitle();

      title.should.equal(
        fi.error_Title + " - TKO-äly ry"
      );
      const contentTitle: string = await browser
  ...
  it(
    "Missing service identifier should show the correct error page (English)",
    async () => {
      await browser.get(
  ...
  for (const service of serviceData) {
    it(
      ...
      async () => {
        ...
        const usernameLabel: string = await browser
          .findElement(By.className("usernameLabel"))
          .getText();
        usernameLabel.should.equal(fi.login_UsernameLabel);

        const passwordLabel: string = await browser
          .findElement(By.className("passwordLabel"))
          .getText();
        passwordLabel.should.equal(fi.login_PasswordLabel);

        const usernamePlaceholder: string = await browser
          .findElement(By.id("username"))
          .getAttribute("placeholder");
        usernamePlaceholder.should.equal(fi.login_UsernamePlaceholder);

        const passwordPlaceholder: string = await browser
          .findElement(By.id("password"))
          .getAttribute("placeholder");

        passwordPlaceholder.should.equal(fi.login_PasswordPlaceholder);

        const acceptInput: string = await browser
          .findElement(By.className("input accept"))
          .getAttribute("value");
        acceptInput.should.equal(fi.login_LoginButton);

        const loginInEnglish: string = await browser
          .findElement(By.className("loginInEnglish"))
          .getText();
        loginInEnglish.should.equal(fi.login_InEnglish);

        const applyToBeAMember: string = await browser
          .findElement(By.className("applyToBeAMember"))
          .getText();
  ...
  for (const service of serviceData) {
    it(
      ...
      async () => {
        ...
        const accept: string = await browser
          .findElement(By.className("accept"))
          .getAttribute("value");
        accept.should.equal(en.login_LoginButton);

        const loginInFinnish: string = await browser
          .findElement(By.className("loginInFinnish"))
          .getText();
  ...
  for (const service of serviceData) {
    it(
      ...
      async () => {
        ...
        const errorText: string = await browser
          .findElement(By.className("error-text"))
          .getText();
        ...
        const acceptValue: string = await browser
          .findElement(By.className("accept"))
          .getAttribute("value");
        ...
        const registerToService: string = await browser
          .findElement(By.className("applyToBeAMember"))
          .getText();
