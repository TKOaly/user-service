...
describe("Login page", () => {
  ...
  for (const service of serviceData) {
    test(
      "On invalid username or password, shows correct error message (Finnish) - " + service.display_name,
      async () => {
        await browser.get("http://localhost:3010/lang/fi/" + service.service_identifier);
  ...
  for (const service of serviceData) {
    test(
      "On invalid username or password, shows correct error message (English) - " + service.display_name,
      async () => {
        await browser.get("http://localhost:3010/lang/en/" + service.service_identifier);
